...
export function normalizeHexColor(hex?: string | null, fallback = REGALCLIC_WALLET_PRIMARY_HEX): string {
  const trimmed = (hex || "").trim();
  if (HEX_COLOR_RE.test(trimmed)) return trimmed.toUpperCase();
  if (HEX_COLOR_RE.test(fallback)) return fallback.toUpperCase();
...
export function resolvePrimaryRgb(hex?: string | null): string {
  const candidate = normalizeHexColor(
    hex || Deno.env.get("REGALCLIC_WALLET_PRIMARY_HEX") || null,
    REGALCLIC_WALLET_PRIMARY_HEX,
  );
  return hexToRgbString(candidate) || "rgb(11,30,63)";
}

export function resolveLabelHex(hex?: string | null): string {
  const candidate = (hex || "").trim();
  if (HEX_COLOR_RE.test(candidate)) return candidate.toUpperCase();
  return REGALCLIC_WALLET_ACCENT_TEAL_HEX;
}

export function resolveLabelRgb(hex?: string | null): string {
  return hexToRgbString(resolveLabelHex(hex)) || "rgb(68,196,161)";
}
...
export function resolvePrimaryHex(hex?: string | null): string {
  const envDefault = Deno.env.get("REGALCLIC_WALLET_PRIMARY_HEX") || REGALCLIC_WALLET_PRIMARY_HEX;
  return normalizeHexColor(hex || envDefault, REGALCLIC_WALLET_PRIMARY_HEX);
}
